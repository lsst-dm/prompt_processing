...
class RunUtilsFunctionTest(unittest.TestCase):
    ...
    def test_get_output_run(self):
        ...
        self.assertEqual(out_chain, f"{instname}/prompt/output-20230122")
        preload_run = get_preload_run(instrument, deploy_id, date)
        self.assertEqual(preload_run, f"{instname}/prompt/output-20230122/NoPipeline/{deploy_id}")
        out_run = get_output_run(instrument, deploy_id, filename, date)
        self.assertEqual(out_run, f"{instname}/prompt/output-20230122/ApPipe/{deploy_id}")
