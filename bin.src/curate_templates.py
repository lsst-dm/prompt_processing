...
"""Curate template_coadd datasets with a new TAGGED collection.
...
def main():
    ...
    if not registered:
        logging.error(
            f"Collection {tagged_collection} already exists before. Aborting."
        )
        sys.exit(1)

    refs = butler.query_datasets(
        "template_coadd", collections=args.collections, where=args.where
    )
    ...
    refs = butler.query_datasets("template_coadd", collections=run_collection)
